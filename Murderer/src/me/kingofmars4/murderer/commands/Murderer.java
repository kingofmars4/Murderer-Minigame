		...
		if (cmd.getName().equalsIgnoreCase("murderer")) {
			...
			case "createmap":
				...
				if (!(args.length == 2)) { p.sendMessage(Messages.pluginPrefix + U.color("&cCorrect usage: &e/murderer createmap (name)")); } else {
					...
					p.sendMessage(Messages.pluginPrefix + U.color("&aSuccesefully created map &e'%map' &aand set your current position has the Game spawn!".replaceAll("%map", args[1])));
				} break;
			...
			case "deletemap":
				if (!p.hasPermission("murderer.staff")) { p.sendMessage(Messages.noPermission); return true; }
				if (!(args.length == 2)) { p.sendMessage(Messages.pluginPrefix + U.color("&cCorrect usage: &e/murderer deletemap (name)")); } else {
					p.sendMessage(Messages.pluginPrefix + U.color("&aStarting map deletion..."));
					if (!GameManager.get().isGame(args[1])) { p.sendMessage(Messages.pluginPrefix + U.color("&e'%map' &cdoes not exists! ".replaceAll("%map", args[1]))); p.sendMessage(Messages.pluginPrefix + U.color("&cCancelling map deltion..."));; return true; }

					GameManager.get().deleteGame(args[1]);
					p.sendMessage(Messages.pluginPrefix + U.color("&aSuccesefully deleted map &e'%map'&a!".replaceAll("%map", args[1])));
				} break;
			...
			case "setspawn":
				...
				if (!(args.length == 3)) { p.sendMessage(Messages.pluginPrefix + U.color("&cCorrect usage: &e/murderer setspawn (map) (number)")); } else {
					if (GameManager.get().isGame(args[1])) {
						...
						if (!U.isInt(args[2])) { p.sendMessage(Messages.pluginPrefix + U.color("&e'%args' &cisnt a number!".replaceAll("%args", id+""))); return true; }
						int n = Integer.parseInt(args[2]);
						if (n<=0) { p.sendMessage(Messages.pluginPrefix + U.color("&e'%args' &cneeds to be a number above 0!".replaceAll("%args", id+""))); return true; }
						
						
						FileMaps.get().set("Maps."+l.getName()+".spawns."+id, U.serializeLoc(p.getLocation()));
						FileMaps.save();
						l.getSpawns().put(id, p.getLocation());
						p.sendMessage(Messages.pluginPrefix + U.color("&aYour current position has been set as &e'%map' &6#%number spawnpoint&a!".replaceAll("%map", l.getName()).replaceAll("%number", n+"")));
			...
			case "setfragmentspawn":
				...
				if (!(args.length == 3)) { p.sendMessage(Messages.pluginPrefix + U.color("&cCorrect usage: &e/murderer setfragmentspawn (map) (number)")); } else {
					if (GameManager.get().isGame(args[1])) {
						...
						p.sendMessage(Messages.pluginPrefix + U.color("&aYour current position has been set as &e'%map' &6#%number fragment spawnpoint&a!".replaceAll("%map", l.getName()).replaceAll("%number", n+"")));
					} else { p.sendMessage(Messages.pluginPrefix + U.color("&cMap &e'%map' &cdoes not exist!".replaceAll("%map", args[1]))); }
				} break;
